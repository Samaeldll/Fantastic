...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (sT->EnableTransmogInfo)
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|tКак работает трасмогрификация", EQUIPMENT_SLOT_END+9, 0);
...
#endif
        player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tСбросить все изменения", EQUIPMENT_SLOT_END+2, 0, "Вы уверены что хотите сбросить все изменения?", 0, false);
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-Undo:30:30:-18:0|tМеню апдейтов", EQUIPMENT_SLOT_END+1, 0);
        ...
        case EQUIPMENT_SLOT_END+4: // Presets menu
            {
                ...
                if (sT->EnableSetInfo)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|tКак работает трансмогрификация сетов", EQUIPMENT_SLOT_END+10, 0);
        ...
        case EQUIPMENT_SLOT_END+6: // view preset
            {
                ...
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Statue_02:30:30:-18:0|tНадеть сет", EQUIPMENT_SLOT_END+5, action, "Выбранные вещи будут изменены.\nХотите продолжить?\n\n"+sT->presetByName[player->GetGUID()][action], 0, false);
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-LeaveItem-Opaque:30:30:-18:0|tУдалить сет", EQUIPMENT_SLOT_END+7, action, "Вы уверены что хотите удалить сет "+sT->presetByName[player->GetGUID()][action]+"?", 0, false);
        ...
        case EQUIPMENT_SLOT_END+8: // Save preset
            {
                ...
                if (canSave)
                    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/GuildBankFrame/UI-GuildBankFrame-NewTab:30:30:-18:0|tСохранить сет", 0, 0, "Введите название сета", cost*sT->SetCostModifier+sT->SetCopperCost, true);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-Undo:30:30:-18:0|tМеню апдейтов", sender, action);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/Ability_Spy:30:30:-18:0|tНазад..", EQUIPMENT_SLOT_END+4, 0);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
    ...
    void ShowTransmogItems(Player* player, Creature* creature, uint8 slot) // Only checks bags while can use an item from anywhere in inventory
    {
        ...
        if (oldItem)
        {
            ...
            ss << std::endl;
              ...
              //Guild-Level-System (Bonus: Transmog)
                if (Guild* guild = player->GetGuild())
                  {
                    if (guild->HasLevelForBonus(GUILD_BONUS_PRICE_TRANSMOG))
                        sT->TokenAmount = 10;
                  } 
            ...
            for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
            {
                ...
                if (sT->GetFakeEntry(oldItem->GetGUID()) == newItem->GetEntry())
                    continue;
                ++limit;
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0)+sT->GetItemLink(newItem, session), slot, newItem->GetGUIDLow(), "Выбранная вещь будет изменена.\nХотите продолжить?\n\n"+sT->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10)+sT->GetItemLink(newItem, session)+ss.str(), price, false);
            }
        ...
        player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tСбросить изменение", EQUIPMENT_SLOT_END+3, slot, "Вы уверены что хотите сбросить даное изменение?", 0, false);
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-Undo:30:30:-18:0|tМеню апдейтов", EQUIPMENT_SLOT_END, slot);
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/Ability_Spy:30:30:-18:0|tНазад..", EQUIPMENT_SLOT_END+1, 0);
        player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
    }
};
