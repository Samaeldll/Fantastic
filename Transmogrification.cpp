...
const char * Transmogrification::GetSlotName(uint8 slot, WorldSession* session) const
{
    switch (slot)
    {
    case EQUIPMENT_SLOT_HEAD      : return  "Шлем";// session->GetTrinityString(LANG_SLOT_NAME_HEAD);
    case EQUIPMENT_SLOT_SHOULDERS : return  "Наплечники";// session->GetTrinityString(LANG_SLOT_NAME_SHOULDERS);
    case EQUIPMENT_SLOT_BODY      : return  "Рубашка";// session->GetTrinityString(LANG_SLOT_NAME_BODY);
    case EQUIPMENT_SLOT_CHEST     : return  "Нагрудник";// session->GetTrinityString(LANG_SLOT_NAME_CHEST);
    case EQUIPMENT_SLOT_WAIST     : return  "Пояс";// session->GetTrinityString(LANG_SLOT_NAME_WAIST);
    case EQUIPMENT_SLOT_LEGS      : return  "Набедренники";// session->GetTrinityString(LANG_SLOT_NAME_LEGS);
    case EQUIPMENT_SLOT_FEET      : return  "Ботинки";// session->GetTrinityString(LANG_SLOT_NAME_FEET);
    case EQUIPMENT_SLOT_WRISTS    : return  "Браслеты";// session->GetTrinityString(LANG_SLOT_NAME_WRISTS);
    case EQUIPMENT_SLOT_HANDS     : return  "Рукавицы";// session->GetTrinityString(LANG_SLOT_NAME_HANDS);
...
void Transmogrification::LoadConfig(bool reload)
{
    ...
    TokenAmount = uint32(sConfigMgr->GetIntDefault("Transmogrification.TokenAmount", 15));
