...
class ArenaTeamRanks : public CreatureScript
{
    private:
        ...
        std::string raceToString(uint8 race) {
            ...
            switch (race)
            {
                ...
                case RACE_GOBLINNEW:        race_s = "Goblin";      break;
                case RACE_WORGEN:           race_s = "Worgen";      break;
                case RACE_PANDARENA:        race_s = "Pandaren";    break;
                case RACE_PANDARENH:        race_s = "Pandaren";    break;
            }
            return race_s;
        }
    ...
    public:
        ...
        bool OnGossipHello(Player *player, Creature *creature) {
            ...
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "Арена 2х2", GOSSIP_SENDER_MAIN, ARENA_2V2_LADDER);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "Арена 3х3", GOSSIP_SENDER_MAIN, ARENA_3V3_LADDER);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "Арена 5х5", GOSSIP_SENDER_MAIN, ARENA_5V5_LADDER);
        ...
        bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 uiAction) {
            ...
            switch(uiAction) {
                ...
                case ARENA_3V3_LADDER:
                {
                    ...
                    if(!result) {
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "До свидания", GOSSIP_SENDER_MAIN, ARENA_GOODBYE);
                ...
                default:
                {
                    if (uiAction > ARENA_START_TEAM_LOOKUP) 
                    {
                        ...
                        std::stringstream buf;
                        buf << "Название тимы: " << name;
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, buf.str(), GOSSIP_SENDER_MAIN, parentOption);
                        buf.str("");
                        buf << "Рейтинг: " << rating << " (место в рейтинге " << rank << ", тима " << type << "х" << type << ")";
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, buf.str(), GOSSIP_SENDER_MAIN, parentOption);
                        buf.str("");
                        buf << "Результат за неделю: " << weekWins << "-" << weekLosses << " (" << weekWinPercentage << " побед), " << (weekWins + weekLosses) << " сыграно"; 
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, buf.str(), GOSSIP_SENDER_MAIN, parentOption);
                        buf.str("");
                        buf << "Результат за сезон: " << seasonWins << "-" << seasonLosses << " (" << seasonWinPercentage << " побед), " << (seasonWins + seasonLosses) << " сыграно"; 
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, buf.str(), GOSSIP_SENDER_MAIN, parentOption);
                        ...
                        if(!members) {
                            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Членов тимы не найдено...?", GOSSIP_SENDER_MAIN, parentOption);
                        } else {
                            uint32 memberPos = 1;
                            ...
                            buf.str("");
                            buf << memberCount << ((memberCount == 1) ? " член" : (memberCount == 2) ? " члена" : (memberCount == 3) ? " члена" : (memberCount == 4) ? " члена" : " членов") << ((memberCount == 1) ? " найден:" : " найдено");
                            ...
                            do {
                                ...
                                buf.str("");
                                buf << "Уровень " << level << " " << race << " " << Class << ", " << personalRating << " персональный рейтинг.";
                                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, buf.str(), GOSSIP_SENDER_MAIN, parentOption);
                                buf.str("");
                                buf << "Неделя: " << weekWins << "-" << weekLosses << " (" << weekWinPercentage << " побед), " << (weekWins + weekLosses) << " сыграно"; 
                                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, buf.str(), GOSSIP_SENDER_MAIN, parentOption);
                                buf.str("");
                                buf << "Сезон: " << seasonWins << "-" << seasonLosses << " (" << seasonWinPercentage << " побед), " << (seasonWins + seasonLosses) << " сыграно"; 
                                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, buf.str(), GOSSIP_SENDER_MAIN, parentOption);
                                memberPos++;
                        ...
                        buf.str("");
                        buf << "Вернуться к " << type << "х" << type << " арене!";
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, buf.str(), GOSSIP_SENDER_MAIN, parentOption);
