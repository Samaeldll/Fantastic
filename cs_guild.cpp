...
public:
    ...
    static bool HandleGuildInfoCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        if (guild)
        {
            ...
            handler->PSendSysMessage("Активные гильд-бонусы:");

            if (guild->GetLevel() > 0)
            {
                if (guild->HasLevelForBonus(GUILD_BONUS_XP_1))
                    handler->PSendSysMessage("XP-Бонус [Ранг 1]");
                if (guild->HasLevelForBonus(GUILD_BONUS_HONOR_1))
                    handler->PSendSysMessage("Honor-Бонус [Ранг 1]");
                if (guild->HasLevelForBonus(GUILD_BONUS_XP_2))
                    handler->PSendSysMessage("XP-Бонус [Ранг 2]");
                if (guild->HasLevelForBonus(GUILD_BONUS_SPEED_1))
                    handler->PSendSysMessage("Speed-Бонус [Ранг 1]");
                if (guild->HasLevelForBonus(GUILD_BONUS_AURA_1))
                    handler->PSendSysMessage("Spell-Бонус [Ранг 1]");
                if (guild->HasLevelForBonus(GUILD_BONUS_AURA_2))
                    handler->PSendSysMessage("Spell-Бонус [Ранг 2]");
                if (guild->HasLevelForBonus(GUILD_BONUS_HONOR_2))
                    handler->PSendSysMessage("Honor-Бонус [Ранг 2]");
                if (guild->HasLevelForBonus(GUILD_BONUS_SPEED_2))
                    handler->PSendSysMessage("Speed-Бонус [Ранг 2]");
                if (guild->HasLevelForBonus(GUILD_BONUS_TALENT))
                    handler->PSendSysMessage("Talent-Бонус");
                if (guild->HasLevelForBonus(GUILD_BONUS_PRICE_TRANSMOG))
                    handler->PSendSysMessage("Transmog-Бонус");
                if (guild->HasLevelForBonus(GUILD_BONUS_PRICE_REFORGING))
                    handler->PSendSysMessage("Reforging-Бонус");
                if (guild->HasLevelForBonus(GUILD_BONUS_BG_BADGE_X2))
                    handler->PSendSysMessage("BG Badge-Бонус");
                if (guild->HasLevelForBonus(GUILD_BONUS_ITEM_TABARD))
                    handler->PSendSysMessage("Item-Бонус");
                if (guild->HasLevelForBonus(GUILD_BONUS_BADGE_ONLINE))
                    handler->PSendSysMessage("Online-Бонус");
            }
            else
                handler->PSendSysMessage("Бонусы отсутствуют.");
