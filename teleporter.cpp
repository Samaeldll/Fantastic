...
uint32 Custom_GetText (unsigned int menu, Player* player)
{
...
void Custom_GetMenu (Player* player, Creature* creature, uint32 Key)
{
        bool ENDMENU = false;
        for(uint32 i = 0; i < TELEPORT_COUNT; i++)
        {
                ...
                if (TeLe[i].menu_id == Key && player->getLevel() >= TeLe[i].level && Custom_FactCheck(player->GetTeam(), i))
                {
                        if (TeLe[i].next_menu_id != 0)
                                player->ADD_GOSSIP_ITEM_EXTENDED(TeLe[i].icon, TeLe[i].name, GOSSIP_SENDER_MAIN, i, "", TeLe[i].cost, false);
                        else
                                player->ADD_GOSSIP_ITEM_EXTENDED(TeLe[i].icon, TeLe[i].name, GOSSIP_SENDER_MAIN, i, ARE_YOU_SURE+TeLe[i].name, TeLe[i].cost, false);
                        ENDMENU = true;
                }
        }
        player->PlayerTalkClass->SendGossipMenu(Custom_GetText(Key, player), creature->GetGUID());
}
...
class TeLe_gossip_codebox : public CreatureScript
{
        ...
        bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 uiAction)
        {
                ...
                player->ModifyMoney(-1 * TeLe[uiAction].cost); // take cash
                ...
                if (Key == 0) // if no next_menu_id, then teleport to coords
                {
                        if (!player->IsInCombat())
                        {
                                ...
                                player->TeleportTo(TeLe[uiAction].map, TeLe[uiAction].x, TeLe[uiAction].y, TeLe[uiAction].z, TeLe[uiAction].o);
                                return true;
                        }
                ...
                Custom_GetMenu(player, creature, Key);
                return true;
        }
};
