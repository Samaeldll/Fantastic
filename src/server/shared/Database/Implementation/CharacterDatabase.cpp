...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_SEL_CREATURE_RESPAWNS_TIME, "SELECT respawnTime FROM creature_respawn WHERE guid = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_CREATURE_RESPAWNS_BONUS_TIME, "SELECT respawnTime, bonus_respawn_time FROM creature_respawn WHERE guid = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_UPD_CREATURE_RESPAWNS_BONUS_TIME, "UPDATE creature_respawn SET respawnTime = ?, bonus_respawn_time = ? WHERE guid = ?", CONNECTION_ASYNC);

    // Player Talent
    PrepareStatement(CHAR_SEL_CUSTOM_TALENT, "SELECT talent_count, guild_talent FROM custom_talent WHERE GUID = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_REP_CUSTOM_TALENT, "REPLACE INTO custom_talent (GUID, talent_count, guild_talent) VALUES (?, ?, ?)", CONNECTION_ASYNC);
