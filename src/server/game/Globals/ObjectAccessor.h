...
class HashMapHolder
{
    public:
        ...
        typedef UNORDERED_MAP<uint64, T*> ObjectInMap;
        ...
        static void InsertCreature(T* o)
        {
            TRINITY_WRITE_GUARD(LockType, i_lock);
            m_objectInMap[o->GetGUIDLow()] = o;
        }

        static void RemoveCreature(T* o)
        {
            TRINITY_WRITE_GUARD(LockType, i_lock);
            m_objectInMap.erase(o->GetGUIDLow());
        }
        ...
        static T* FindCreature(uint64 guid)
        {
            TRINITY_READ_GUARD(LockType, i_lock);
            typename ObjectInMap::iterator itr = m_objectInMap.find(guid);
            return (itr != m_objectInMap.end()) ? itr->second : NULL;
        }
    ...
    private:
        ...
        static ObjectInMap m_objectInMap;
};
...
class ObjectAccessor
{
    ...
    public:
        ...
        template<class T> static void AddCreature(T* object)
        {
            HashMapHolder<T>::InsertCreature(object);
        }

        template<class T> static void RemoveCreature(T* object)
        {
            HashMapHolder<T>::RemoveCreature(object);
        }
