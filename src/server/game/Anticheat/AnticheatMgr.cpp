...
void AnticheatMgr::BuildReport(Player* player,uint8 reportType)
{
   ...
   if (m_Players[key].GetTotalReports() >= 15)
      {
         player->AddAura(2647, player);
         //player->TeleportTo(player->GetMapId(), m_Players[key].GetLastMovementInfo().pos.GetPositionX(), m_Players[key].GetLastMovementInfo().pos.GetPositionY(), m_Players[key].GetLastMovementInfo().pos.GetPositionZ(), m_Players[key].GetLastMovementInfo().pos.GetOrientation());
         player->GetSession()->SendNotification("|cffADFF2FВы были заморожены в подозрении на читерство");
      }
}
