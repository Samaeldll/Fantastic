...
void Player::Update(uint32 p_time)
{
    ...
    if (IsHasDelayedTeleport() && IsAlive())
        TeleportTo(m_teleport_dest, m_teleport_options);
           ...
           //Guild-Level-System (Bonus: Aura 2)
             if (Guild* guild = GetGuild())
              {
                if (guild->HasLevelForBonus(GUILD_BONUS_AURA_2))
                  if (GetZoneId() == 210 || GetZoneId() == 3483 || GetZoneId() == 2817 || GetZoneId() == 17 || GetZoneId() == 46)
                     {
                      if (!HasAura(90319))
                         AddAura(90319, this);
                     }
                  else if (HasAura(90319)) RemoveAura(90319);
              }
}
...
bool Player::RewardHonor(Unit* victim, uint32 groupsize, int32 honor, bool pvptoken)
{
    ...
    if (honor_f <= 0)
    {
        ...
        if (Player* plrVictim = victim->ToPlayer())
        {
          ...
          /*if (GetSession()->GetRemoteAddress() == plrVictim->GetSession()->GetRemoteAddress() || plrVictim->GetMaxHealth() < 500000)
           {
              GetSession()->SendNotification("|cffADFF2FРђРЅС‚Рё-С„Р°СЂРј СЃРёСЃС‚РµРјР°.");
              return false;
           }*/
    ...
    //Guild-Level-System (Bonus: Badge kill)
    if (Guild* guild = GetGuild())
    {
        srand(time(0));
        int chance = rand() % 100+1;
        if (guild->HasLevelForBonus(GUILD_BONUS_BG_BADGE_X2))
            if (chance <=20)
                count = 2;
    }

            if (AddItem(itemId, count))
                ChatHandler(GetSession()).PSendSysMessage("|cffADFF2FР’Р°Рј Р±С‹Р»Рѕ РІС‹РґР°РЅРѕ РІРѕР·РЅР°РіСЂР°Р¶РґРµРЅРёРµ Р·Р° СѓР±РёР№СЃС‚РІРѕ РёРіСЂРѕРєР°.");
...
void Player::_ApplyItemBonuses(ItemTemplate const* proto, uint8 slot, bool apply, bool only_level_scale /*= false*/)
{
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_STATS; ++i)
    {
        ...
        else
        {
            ...
            if(lowGUID && sObjectMgr->itemFakeStatStore[lowGUID].find(statType) != sObjectMgr->itemFakeStatStore[lowGUID].end())
                val += sObjectMgr->itemFakeStatStore[lowGUID][statType];
...
void Player::RewardQuest(Quest const* quest, uint32 reward, Object* questGiver, bool announce)
{
    ...
    //Guild-Level-System (Bonus: XP)
    if (Guild* guild = GetGuild())
    {
      if (quest->GetQuestId() == 100078)
         {
            uint32 guild_xp = 50000;
             //GuildXP-Bonus
            if (guild->HasLevelForBonus(GUILD_BONUS_XP_1))
               guild_xp = uint32(guild_xp*1.05f);
            if (guild->HasLevelForBonus(GUILD_BONUS_XP_2))
               guild_xp = uint32(guild_xp*1.1f);

             //QuestXP for the Guild
               guild->GiveXp(guild_xp);
         }
    }
...
bool Player::ModifyMoney(int32 amount, bool sendError /*= true*/)
{
    ...
    if (amount < 0)
    {
        if (GetMoney() > uint32(-amount))
            SetMoney(GetMoney() + amount);
        else if (GetItemCount(90030))
        {
            DestroyItemCount(90030, 1, true);
            SetMoney(MAX_MONEY_AMOUNT + (GetMoney() + amount));
        }
        else
            SetMoney(0);
    }
    else
    {
        if (GetMoney() < uint32(MAX_MONEY_AMOUNT - amount))
            SetMoney(GetMoney() + amount);
        else if (AddItem(90030, 1))
            SetMoney(amount - (MAX_MONEY_AMOUNT - GetMoney()));
        else
        {
            if (sendError)
...
bool Player::CharSelectAndReplace() const
{
      uint32 item_talent_info = 0;
      uint32 guild_talent_info = 0;
          ...
          {
                  Field* fields = result->Fetch();
                  item_talent_info = fields[0].GetUInt32();
                  guild_talent_info = fields[1].GetUInt32();
          }

          if (item_talent_info >=5)
             return false;
 
          stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_CUSTOM_TALENT);
          stmt->setUInt32(0, GetGUIDLow());
          stmt->setUInt32(1, item_talent_info+1);
          stmt->setUInt32(2, guild_talent_info);
          CharacterDatabase.Execute(stmt);

          return true;
} 

uint32 Player::FreeTalent() const
{
      uint32 it_count_t = 0;
      uint32 guild_tal = 0;
          ...
          {
                  Field* fields = result->Fetch();
                  it_count_t = fields[0].GetUInt32();
                  guild_tal = fields[1].GetUInt32();
          }

        return it_count_t + guild_tal;
}
...
uint32 Player::CalculateTalentsPoints() const
{
    ...
    if (getClass() != CLASS_DEATH_KNIGHT || GetMapId() != 609)
        return uint32(base_talent * sWorld->getRate(RATE_TALENT)+ FreeTalent());
    ...
    return uint32(talentPointsForLevel * sWorld->getRate(RATE_TALENT) + FreeTalent());
}
