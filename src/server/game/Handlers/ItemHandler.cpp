...
void WorldSession::HandleItemQuerySingleOpcode(WorldPacket& recvData)
{
    ...
    if (pProto)
    {
     ...
     if (pProto->CustomCosts || pProto->CustomReqArena || pProto->CustomReqHonor)
        sCustomItem->HandleCustomItemDescription(pProto, Description);
...
void WorldSession::SendListInventory(uint64 vendorGuid, uint32 vendorEntry)
{
    ...
    VendorItemData const* items = vendorEntry ? sObjectMgr->GetNpcVendorItemList(vendorEntry) : vendor ? vendor->GetVendorItems() : NULL;
    if (!items)
    {
    ...
    for (uint8 slot = 0; slot < itemCount; ++slot)
    {
        if (VendorItem const* item = items->GetItem(slot))
        {
            if (ItemTemplate const* itemTemplate = sObjectMgr->GetItemTemplate(item->item))
            {
                ...
                // TODO: Implement player specific vendor stocks
                uint32 leftInStock = !vendor || !item->maxcount ? 0xFFFFFFFF : vendor->GetVendorItemCurrentCount(item);
