...
void WorldSession::SendTrainerList(uint64 guid, uint32 vendor_entry)
{
    std::string str = GetTrinityString(LANG_NPC_TAINER_HELLO);
    SendTrainerList(guid, str, vendor_entry);
}

void WorldSession::SendTrainerList(uint64 guid, const std::string& strTitle, uint32 vendor_entry)
{
    TC_LOG_DEBUG("network", "WORLD: SendTrainerList");

    Creature* unit = GetPlayer()->GetNPCIfCanInteractWith(guid, vendor_entry ? 1 : UNIT_NPC_FLAG_TRAINER);
    if (!unit)
    {
    ...
    TrainerSpellData const* trainer_spells = vendor_entry ? sObjectMgr->GetNpcTrainerSpells(vendor_entry) : unit->GetTrainerSpells();
    if (!trainer_spells)
    {
    ...
    SetCurrentTrainer(vendor_entry);
...
void WorldSession::HandleTrainerBuySpellOpcode(WorldPacket& recvData)
{
    ...
    Creature* unit = GetPlayer()->GetNPCIfCanInteractWith(guid, GetCurrentTrainer() ? 1 : UNIT_NPC_FLAG_TRAINER);
    if (!unit)
    {
