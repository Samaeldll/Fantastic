            ...
            if (Player* player = ObjectAccessor::FindPlayer(itr->first))
                {
                   player->ResetAllPowers();
                    ...
                    //Guild-Level-System (Bonus: Aura)
                    if (Guild* guild = player->GetGuild())
                     {
                       if (guild->HasLevelForBonus(GUILD_BONUS_AURA_1))
                           player->AddAura(90321, player);
                     }
                }
    }
...
void Battleground::EndBattleground(uint32 winner)
{
    ...
    if (isArena() && isRated())
    {
        ...
        can_award.insert(std::pair<uint32, Player *> (winnerArenaTeam->GetId(), __player));
        can_award.insert(std::pair<uint32, Player *> (loserArenaTeam->GetId(), __player));
    ...
    {
        uint32 team = itr->second.Team;
        uint32 a_team = (isArena() && isRated() ? GetArenaTeamIdForTeam(team) : 0);
        ...
        if (isArena() && isRated() && can_award.find(a_team) != can_award.end())
                 can_award.insert(std::pair<uint32, Player *>(a_team, player));
        ...
        if (isArena() && isRated() && winnerArenaTeam && loserArenaTeam && winnerArenaTeam != loserArenaTeam)
        {
            if (team == winner)
            {
                if (player->HasItemCount(sWorld->getIntConfig(CONFIG_CUSTOM_ARENA_AWARDS_CHECK_ID)))
                 p_win++;
    ...
    if (isArena() && isRated() && winnerArenaTeam && loserArenaTeam && winnerArenaTeam != loserArenaTeam)
    {
        if (sWorld->getBoolConfig(CONFIG_CUSTOM_ARENA_AWARDS))
            for (std::multimap<uint32, Player * >::iterator a_it = can_award.begin(); a_it != can_award.end(); ++a_it)
            {
                if (!(*a_it).second)
                    continue;

                if ((*a_it).first == winnerArenaTeam->GetId())
                      if (p_win == 2)
                       (*a_it).second->AddItem(sWorld->getIntConfig(CONFIG_CUSTOM_ARENA_AWARDS_WINNER_ITEM_ID), sWorld->getIntConfig(CONFIG_CUSTOM_ARENA_AWARDS_WINNER_ITEM_COUNT));
                else
                  {
                    if (p_los == 2)
                     (*a_it).second->AddItem(sWorld->getIntConfig(CONFIG_CUSTOM_ARENA_AWARDS_LOOSER_ITEM_ID), sWorld->getIntConfig(CONFIG_CUSTOM_ARENA_AWARDS_LOOSER_ITEM_COUNT));
                  }
            }
...
void Battleground::HandleKillPlayer(Player* victim, Player* killer)
{
    ...
    if (killer)
    {
        ...
        if (victim->GetGUID() == playerTarget)
            RewardPlayerKillTarget(killer);
...
void Battleground::SetPlayerTarget()
{
    if (playerTarget > 0) // We already have player
        return;

    if (GetPlayersCountByTeam(HORDE) < 3 || GetPlayersCountByTeam(ALLIANCE) < 3) // Min players per team
        return;
    ...
    for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
        if (Player* player = ObjectAccessor::FindPlayer(itr->first))
        {
            if ((int)rand_chance() > 50)
                players.push_back(player);
            else
                players.push_front(player);
        }

    for(std::list< Player* >::iterator itr2 = players.begin(); itr2 != players.end(); ++itr2)
        if ((int)rand_chance() > 70)
        {
            TPlayer = *itr2;
            break;
        }
    ...
    TPlayer->SetDisplayId(TargetMorph);
    TPlayer->SetFloatValue(OBJECT_FIELD_SCALE_X, 1.5f);

    playerTarget = TPlayer->GetGUID();

    SendWarningToAll(28636, TPlayer->GetName().c_str());
    /*
        INSERT INTO `trinity_string` (`entry`, `content_default`) VALUES ('28636', 'Player: %s - is target. '); 
    */
}

void Battleground::RewardPlayerKillTarget(Player* killer)
{
    RemovePlayerTarget(); // remove target
    killer->AddItem(90006, 1);
    SetPlayerTarget(); // Set new target
}

void Battleground::RemovePlayerTarget()
{
    if (playerTarget == 0)
        return;
    
    Player* pl = ObjectAccessor::FindPlayer(playerTarget);
    ...
    pl->DeMorph();
    pl->SetFloatValue(OBJECT_FIELD_SCALE_X, 1.0f);
}
