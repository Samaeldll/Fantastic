        ...
        if (lower.find(moton) != std::string::npos)
        {
			...
			for (SessionMap::const_iterator itr = ss.begin(); itr != ss.end(); ++itr) 
				if (itr->second->GetSecurity() > 0) 
					str = "|cFFFF0000[ Анти Спам ]|cFF00FFFF [ |cFF60FF00" + std::string(player->GetName()) + "|cFF00FFFF ] [ |cFF60FF00" + lower + "|cFF00FFFF ] Check it!"; 
			WorldPacket data(SMSG_NOTIFICATION, (str.size()+1)); 
			data << str; 
			sWorld->SendGlobalGMMessage(&data); 
			
            ChatHandler(player->GetSession()).PSendSysMessage("|cFFFF0000[ Анти Спам ] : Отправленное вами сообщение противоречит правилам Сервера");
            return;
        }
    }
...
{ 
		...
		{ 
				...
				for (SessionMap::const_iterator itr = ss.begin(); itr != ss.end(); ++itr) 
						if (itr->second->GetSecurity() > 0) 
								str = "|cFFFFFF00[ Анти Фарминг ]|cFF00FFFF[ |cFF60FF00" + std::string(killer->GetName()) + "|cFF00FFFF ] Проверь это!"; 
				WorldPacket data(SMSG_NOTIFICATION, (str.size()+1)); 
				data << str; 
				sWorld->SendGlobalGMMessage(&data); 
		} 
